...
and is one of the eldest and most important results
in the mathematical theory of \textit{functional analysis}.
...
\subparagraph{}
Here $P_{\lambda}$ is a member
of the family
$\{P_{\lambda}\}_{\lambda \in \R}$
of \textit{projection operators} in $\hilH$,
parametrized by the real numbers.
The family $P$ itself is called a \textit{spectral measure},
and the integral in \eqref{ST_1} means to integrate
the identity mapping $\1 \: \lambda \xeof \R \mapsto \lambda$
...
\subparagraph{}
We basically shall go ahead in to achieve this theorem step by step
by giving well-understandable explanations of all the upcoming concepts and
statements.
...
\paragraph{}
In the following we deduce an important criterion
how to prove self-adjointness of a symmetric operator,
usually called the \textit{range condition}:

\begin{theorem}\label{RANGE_CONDITION1}
Let $\linA$ be symmetric and closed
with dense domain $\domain{\linA}$ in $\hilH$ such that
\begin{equation}\label{EQ_RANGE_CONDITION}
\range{\linA + i} \= \range{\linA - i} \= \hilH.
\end{equation}
Then $\linA$ is self-adjoint.
\end{theorem}

\Proof:
Since $\linA \prec \linA^*$,
we have only to show $\linA^* \prec \linA$, or equivalently,
$y \in \domain{\linA^*}$ implies $y \in \domain{\linA}$.

Now,
since $(\linA-i)^*x = (\linA^* +i)x$ for $x \in \domain{\linA}$,
we have
\[
\scalar{\,(\linA-i)x}{y} = \scalar{\linA x}{y} -i\scalar{x}{y} =
\scalar{x}{\linA^*y} -\scalar{x}{-iy} = \scalar{x}{\,(\linA^* +i)y}.
\]
But since $\range{\linA+i} = \hilH$,
there exists $z \in \domain{\linA}$ such that
$
(\linA + i)z = (\linA^* +i)y.
$
...
\]
But since in addition $\range{\linA-i} = \hilH$, we obtain
$\scalar{u}{y-z} = 0$ for every vector $u := (\linA-i)x \in \hilH$,
which implies $y=z$.
\qed

\paragraph{}
It should be noticed
that the imaginary unit $i$ in \eqref{EQ_RANGE_CONDITION}
may be replaced by any other number
$\lambda \in \C \setminus \R$,
since the images of the operators $\linA \pm \lambda \1$
are locally constant for all numbers $\lambda$ with $\im \lambda \neq 0$.

\subparagraph{}
On the other hand,
the following theorem shows that that the reverse statement is also true:

\begin{theorem}\label{RANGE_CONDITION2}
Let $\linA$ be self-adjoint.
Then $\range{\linA - \lambda} = \hilH$ for each $\lambda \in C$ with
$\im \lambda \neq 0$.
\end{theorem}

\Proof:
We first show that $\range{\linA - \lambda}$ lies dense in $\hilH$,
or equivalently, that $\kernel{\linA^* - \overline{\lambda}} = \{0\}$.
We compute for $y \in \kernel{\linA^* - \overline{\lambda}}$
\[
\im \overline{\lambda} \norm{y}^2 =
\im (\overline{\lambda} \scalar{y}{y}) =
\im \scalar{\overline{\lambda}y}{y} =
\im \scalar{\linA^* y}{y} =
\im \scalar{\linA y}{y} = 0,
\]
since $\scalar{\linA y}{y}$ is real.

To the end,
remember from the previous subsection
that $\linA-\lambda$ has closed range for $\im \lambda \neq 0$,
...
let $\zeta \in \C$ such that $\im \zeta \neq 0$.
Then $\zeta$ cannot be an eigenvalue of $\linA$, since $\linA$ is symmetric,
and $\linA-\zeta$ is injective.
But we also know from theorem \ref{RANGE_CONDITION2}
that $\range{\linA-\zeta} = \hilH$,
so $\linA-\zeta$ is also surjective.
Hence $\zeta$ belongs to the resolvent set $\rho(\linA)$,
which implies the assertion.

The resolvent estimation \eqref{RESOLVENT_ESTIMATE} is a special form
of a more general estimate,
quoting that the norm of $R(\linA,\zeta)$ is bounded by the distance
$\delta(\zeta,\sigma(\linA)$
between the singleton $\{\zeta\}$ and the spectrum $\sigma(\linA)$
according to
\[
\norm{R(\linA,\zeta)} \leq \frac{1}{\delta(\zeta,\sigma(\linA))}.
\qed
\]
...
Then by a \textit{projection-valued measure}
is meant a mapping $P \: \Borel{\R} \xto \BO{\hilH}$,
