...
\begin{itemize}[leftmargin=40pt,topsep=2pt,itemsep=2pt]
\item[(N-1)]
\,$\norm{x} \, \geq \, 0$,
...
or by the \textit{metric} topology $\topT := \topT_d$,
...
the \textit{convergence} of a series and many others
are meaningful in the setting of normed linear spaces.
Notice
that equivalent norms on $\banX$ give rise to one and the same topology.
For example,
if $\banX$ is a finite-dimensional linear space,
then all norms on $\banX$ are equivalent
and consequently there is exactly one topology induced by these norms.
...
which is a special kind of mapping between normed linear spaces
...
\]
for $x,y \xeof \banX$ and $\lambda \xeof \F$.
If $\iota$ is even surjective,
it is said to be a \textit{linear isomorphism}
between the spaces $\banX$ and\,$\banY$.
Then \textsc{Banach}\textit{'s isomorphism theorem} claims that
the inverse mapping $\iota^{-1}$ is also an embedding,
so $\iota$ and $\iota^{-1}$ both constitute linear homeomorphisms
between these spaces.
Furthermore,
the embedding $\iota$ is denoted a \textit{norm-isomorphism},
...
\paragraph{} %- summary on continuous and closed linear operators
In conclusion,
...
\subparagraph{}
It should be clear
that every self-adjoint or skew-adjoint or unitary operator is normal.
...
Then for $\zeta \xeof \rho(\linA)$
$\sigma(R(\linA,\zeta)) \setminus \{0\} \eq
\{(\lambda-\zeta)^{-1} \: \lambda \xeof \sigma(\linA)\}$.
\end{theorem}

\subsection{} %-- various kinds of spectra
%------------------------------------------------------------------------------%
We next will have a look at the spectrum 
of a linear operator $\linA$ acting in a Banach space $\banX$.
First notice that the spectrum of $\linA+\zeta$ ($\zeta \xeof \C$) is given by
$\{\lambda+\zeta \: \lambda \xeof \sigma(\linA)\}$.
Here are the possible reasons for $\lambda \xeof \sigma(\linA)$ 
that prevent the operator $\linA {-} \lambda$ from being bijective:
...
\paragraph{} %- bounded below theorem
The \textit{bounded-below theorem} provides a fundamental assertion
in operator theory
and is helpful to verify closeness of the set $\range{\linA}$:
\newpage
...
hence the upper and lower complex half-plane belong to $\rho(\linA)$,
and the spectrum $\sigma(\linA)$ forms a subset of the real line in this case.
\end{itemize}
...
Then any value $\lambda$ belongs to $\sigma_{ess}(\linA)$,
if there is a sequence $\{x_n\}_{n \in \N}$ of orthonormal elements in $\hilH$
...
is nothing but the set of \textit{isolated} eigenvalues of $\linA$
with finite geometric multiplicity.
...
and serves as the main tool to verify this feature:

\begin{theorem}\label{RANGE_CONDITION}
A densely defined, closed and symmetric operator $\linA$
acting in $\hilH$ is self-adjoint
...
\subparagraph{}
If it is even possible to find $\zeta \xeof \R$
fulfilling the range condition $\range{\linA {+} \zeta} \eq \hilH$,
then of course $\linA$ also proves to be self-adjoint.
...
iff there is $\zeta \xeof \C$ such that
\[
\overline{\range{\linA + \zeta}} \=
\overline{\range{\linA + \overline{\zeta}}} \= \hilH.
\]
\end{theorem}
...
self-adjoint
iff
the adjoint $\linA^*$ is symmetric, too.
...
linear operators.
Remember from \textsc{Lebesgue}'s integration theory
...
The initial-value problems under consideration in this section
are the homogeneous Cauchy problems of first and second order
and the abstract {\SCHROEDINGER} equation.
We notice in advance
that they can be solved also by semigroup methods (see sections 9,\,10 and 11).
...
Let us start with the concept of \textit{linear partial differential operator}
and continue with the consideration of specific problems
associated with this kind of mapping.
...
\end{equation}
for $x \xeof \clG$,
the topological closure of a \textit{domain} $\G$,
which is nothing but an open and connected subset of an Euclidean space $\Rd$.
...
in \eqref{EVEN_ORDER_LPDO} are real-valued.
Moreover,
...
and $\BDO_0,\ldots,\BDO_{m-1}$ are \textit{boundary operators}
of order $m_k \leq 2m{-}1$,
each of them given by the expression
\[
\BDO_k(x,u) \eqdef
...
it is sufficent that the real or complex-valued functions $b_{k,\alpha}$
...
that maps any sufficiently differentiable function $u \: \clG \xto \C$
...
is the \textit{homogeneous Dirichlet system}
...
If the set of boundary conditions in problem \eqref{LEBVP} is a Dirichlet system,
then we denote any solution $u \: \clG \xto \C$ \textit{classical},
if
\begin{itemize}[leftmargin=9mm]
...
and governed by a linear partial differential operator $\A$ of second order
...
on which (BC-1), (BC-2) and (BC-3) are prescribed.
In this case we want to speak of \textit{mixed} boundary value problems.
...
Let $\A$ be a linear partial differential operator of order $2m$ be given
...
We then denote the problem \eqref{LEBVP} \textit{elliptic},
if the operator $\A$ is so.
...
then any elliptic differential operator is properly elliptic.

\item[(E-3)]
The concept of ellipticity is often not good enough
to obtain satisfying results.
Hence the expression $\A$ of even order $2m$ 
...
of what is meant by an {uniformly strongly elliptic} differential operator.
One easily computes
...
for the differential operators under consideration.
...
\end{equation}
Then by a \textit{realization} of $\A$ in some
Banach spaces $\mathbb{E}(\G)$ and $\mathbb{E}(\G)$
of functions $u \: \G \xto \C$ is meant any linear operator
\[
\linA: \domain{\linA} \subset \mathbb{E}(G) \xto \mathbb{F}(G)
\]
with domain of definition $\domain{\linA}$.
The Banach space $\mathbb{E}(\G)$ should at least contain
the space $\TF$ of test functions\footnote{
...
$\A \, u$ is well-defined by
$\linA u \eq \A \, u$ and of course
$\A \, u \xeof \mathbb{F}(\G)$ holds.
Clearly,
our first condition means that the set $\domain{\linA}$
contains only functions $u \: \G \xto \C$
being sufficiently often differentiable in some sense.
Since $\domain{\linA}$ includes the space $\TF$,
the subspace $\domain{\linA}$ lies dense in $\mathbb{E}(\G)$
and $\linA$ appears as a densely defined operator
with domain space $\mathbb{E}(\G)$ and range space $\mathbb{F}(\G)$.
One often choses $\mathbb{F}(\G) \eq \mathbb{E}(\G)$,
so $\linA$ is an operator purely acting in $\mathbb{E}(\G)$
and we may apply concepts and results from spectral theory.

\paragraph{} %- content of the section
In the second part of this section we want to present some results
in applied functional analysis {\wrt} linear differential operators.
To be precise, we want to point out that

\begin{itemize}[leftmargin=2mm,itemsep=4pt]
\item[-] 
for a linear partial differential expression $\A$
with differentiable coefficients up to a certain order
there can be defined densely defined and closable rsp. closed 
realizations,
acting either in the {\HOELDER} spaces $C^{k,\gamma}(\G)$
($k \xeof \N_0$, $0 < \gamma \leq 1$)
or in the Lebesgue spaces $\Lp$ ($1 \leq p < \infty$);

\item[-] 
using \textit{elliptic estimates},
the realization of a strongly elliptic linear differentiable operator
acting in Sobolev spaces
and restricted by homogeneous Dirichlet boundary conditions
is graph-closed;

\item[-] 
the study on realizations $\linA$ of a differential operator $\A$
can be supported by that of their adjoints $\linA^*$,
which are determined by so-called \textit{adjointness theorems};

\item[-] 
by starting with quasi-coercive sesquilinear forms,
induced by strongly elliptic differential expressions,
we can obtain realizations in Hilbert space $\Lh$,
which prove to be densely defined and closed;

\item[-] 
\textit{weak solutions} of elliptic boundary value problems exist and are unique,
supposed the governing sesquilinear form is the Dirichlet form
of a strongly elliptic linear differential operator;

\item[-] 
the realizations of a regularly elliptic expression $\A$
in Hilbert space $\Lh$ fulfilling homogeneous Dirichlet conditions are Fredholm,
so the alternative theorem \eqref{FREDHOLM_ALTERNATIVE_THEOREM}
may be applied to the related boundary value problem $\A \, u \eq f$;

\item[-] 
strongly elliptic expressions give rise to m-accretive and 
sectorial realizations in Hilbert space $\Lh$,
...
give rise to self-adjoint realizations in Hilbert space $\Lh$;
\end{itemize}

\subparagraph{}
This enumeration is of course only a small selection of results,
but may give a good impression
how functional analysis tools help to have insight into these problems.
Let us turn to the details of the mentioned assertions:
...
\def\TITLEE{Weak solutions of elliptic boundary value problems.}
...
We basically want to distinguish between two fundamental classes
...
Notice that the domain of definition $\domain{\linA}$
of the linear operator $\linA$ considered in theorem \ref{L2_CLOSABILITY_THEOREM}
...
for the domain of definition of its closure $\overline{\linA}$.
...
in opposite to our previously given definition of this concept.
Using this definition,
we consider the linear operators $\linA_{min}$ and $\linA_{max}$
also as realizations of $\A$.
...
Asis quoted in \cite{Pa} by lemma 7.3.4,
we have 
...
%-- weak solutions of elliptic boundary boundary value problems
%------------------------------------------------------------------------------%
\subsubsection{V. \TITLEE}
The variational approach to stationary (and also evolutional)
boundary value problems provides the concept of weak solution,
...
Given a differential expression $\A$,
Remember the definition of the formally adjoint expression $\A^{\dag}$.
We then denote a linear differential expression $\A$
\textit{formally self-adjoint} in case of $\A^{\dag} \eq \A$.
If $\A$ has constant coefficients,
...
In the following we collect a couple of formally self-adjoint linear
differential expressions and describe some self-adjoint $L^2$-realizations:
...
12. APPLICATIONS TO LINEAR DIFFERENTIAL OPERATORS
    ...
    realizations in the spaces C^{k,\gamma}(G) and L^p(G)
    the closeness of strongly elliptic Dirichlet operators
    formally adjoint expressions and adjointness theorems
    Gardings's inequality and quasi-coercive Dirichlet forms
