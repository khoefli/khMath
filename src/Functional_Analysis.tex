...
\centerline{\huge{\textbf{Functional Analysis}}}
...
and finally of one section concerning with the applications
to linear partial differential equations,
which arise as abstract models of problems in mathematical physics
...
In our treatise we shall consider exclusively with the linear part,
hence the fundamental concept of {"linear operator"}
plays a central role in our considerations.
Especially what historically is meant by the concept of unbounded operator
...
and their boundary and initial-value problems.
...
and especially {\wrt} infinite-dimensional linear spaces.
...
\centerline
{
\begin{tabular}{C{9mm}L{87mm}R{6mm}}
%------------------------------------------------------------------------------%
   I. & Topological and Normed Linear Spaces                   \Df &   1 \\ % 19
  II. & Linear Operators in Banach Spaces                      \Df &  19 \\ %  8
 III. & Calculus of Vector-Valued Functions                    \Df &  27 \\ % 11
  IV. & Adjoint Spaces and Operators                           \Df &  38 \\ % 15
   V. & Normally Solvable Operators                            \Df &  53 \\ %  6
  VI. & General Spectral Theory                                \Df &  59 \\ % 12
 VII. & Commutative Banach Algebras                            \Df &  71 \\ % 19
VIII. & Linear Operators in Hilbert Space                      \Df &  90 \\ %  7
  IX. & Spectral Theory of Self-Adjoint Operators              \Df &  98 \\ % 15
   X. & Semigroup Theory and Evolution Equations               \Df & 113 \\ %  9
  XI. & Coercive Sesquilinear Forms                            \Df & 122 \\ % 13
 XII. & Applications to Linear Differential Operators          \Df & 135 \\ % 32
%------------------------------------------------------------------------------%
\end{tabular}
}
...
and Sobolev spaces will be introduced in subsequent sections
after having available the notion of \textit{adjoint space} (section II)
and methods for integration and differentiation
of vector-valued functions (section III).
\end{comment}
...
of this tree.
For the notion of \textit{reflexive space} we refer to section IV.
...
\part{\TitleII}
%------------------------------------------------------------------------------%
The aim of this second section
...
This leads to the concepts of \textit{continuous}, \textit{compact},
...
between $\banX$ and $\banY$
(in the sense of general topological linear spaces).
...
Notice that the notion of embedding can be introduced
for more general topological linear spaces,
but the validness of the isomorphism theorem may be lost in this case.

\subparagraph{}
A well-known example for a norm-isomorphism is the \textit{Riesz map}
acting between a complex Hilbert space and its conjugate space.
Various kinds of \textit{Fourier transformation} are further examples
of continuous linear operators acting on Lebesgue spaces $L^p(X,\mu)$.
...
We collect further properties of compact linear operators:
...
\subparagraph{} %- finite-rank operators
...
The closed graph theorem is an immediate consequence
...
too,
and forms a homeomorphism between the spaces $\banX$ and $\banY$
(this is \textit{bounded inverse theorem} for continuous operators).
\end{theorem1}
...
and can be derived from \textsc{Baire}'s category theorem,
which is a fundamental assertion in general topology.
...
where $\linA$ is assumed to be closed and
acting between Banach spaces $\banX$ and $\banY$,
we obtain the following concepts:
...
that if the linear operator $\linA$ is closed,
then property (d) implies property (e) by the closed graph theorem.
...
%------------------------------------------------------------------------------%
% III. CALCULUS OF VECTOR-VALUED FUNCTIONS ---
%------------------------------------------------------------------------------%
...
\def\TitleIII{Calculus of Vector-Valued Functions}
\newpage
\thispagestyle{empty}
...
\begin{equation}\label{III_HOMOGENEOUS_CAUCHY_PROBLEM_1}
\begin{cases}[1.2]
\quad x'(t) \+ \linA \, x(t) \= \0,\\ 
...
The evolution equation \eqref{III_HOMOGENEOUS_CAUCHY_PROBLEM_1} with prescribed
...
\begin{equation}\label{III_HOMOGENEOUS_CAUCHY_PROBLEM_2}
\begin{cases}[1.2]
\quad x''(t) \+ \linA \, x(t) \= \0,\\
\quad x(0)  \= x_0, \\
\quad x'(0) \= x_1,\\
...
a \textit{classical solution} of \eqref{III_HOMOGENEOUS_CAUCHY_PROBLEM_1},
...
a \textit{mild solution} of \eqref{III_HOMOGENEOUS_CAUCHY_PROBLEM_1} if
\[
...
First,
the homogeneous Cauchy problem \eqref{III_HOMOGENEOUS_CAUCHY_PROBLEM_1}
...
\eqref{III_HOMOGENEOUS_CAUCHY_PROBLEM_2}).

\subparagraph{}
Let us finally mention that problem \eqref{III_HOMOGENEOUS_CAUCHY_PROBLEM_1}
...
\eqref{III_HOMOGENEOUS_CAUCHY_PROBLEM_1} at time $t > 0$.
...
We first review the notion of classical and mild solution:
...
if $\ldots$ % waiting for implementation
\end{itemize}
...
\bibitem{III_DU} J.\,Diestel, J.\,J.\,Uhl:
\textit{Vector Measures}.
American Mathematical Soc., 1977.

\bibitem{III_EN} K.-J.\,Engel, R.\,Nagel:
...
%------------------------------------------------------------------------------%
%  IV. ADJOINT SPACES AND OPERATORS ---
%------------------------------------------------------------------------------%
...
\newpage
\thispagestyle{empty}
\fancyhead[C] {\small{\textsc{IV. Adjoint Spaces and Operators}}}
\part{Adjoint Spaces and Operators}
%------------------------------------------------------------------------------%
...
\subparagraph{}
The main objects in this section are \textit{continuous linear functionals}
on $\banX$,
that is,
...
\section{Bidual Spaces and Reflexivity}
%------------------------------------------------------------------------------%
Notice that if a linear space $\banX$ is normed,
...
the {adjoint space} $\banX^*$ is complete,
which can easily be proved.
Moreover,
let for a given normed linear space $\banX$ the \textit{bidual space}
defined by $\banX^{**} := (\banX^*)^*$,
which also forms a complete normed linear space.
Then there is a natural embedding
$\iota\: \banX \hookrightarrow \banX^{**}$,
defined by $\iota(x) := \phi_x$ for $\phi_x \xeof \banX^*$,
where $\phi_x(f) := f(x)$ for $f \xeof \banX^*$.
The mapping $\iota$ proves to be linear, continuous and injective.
...
On the other hand,
nothing can be said whether the embedding $\iota$ is onto.

\paragraph{} %- weak topologies
Remember that a normed linear space $\banX$ owns a specific topology,
...
but this is in general not true in arbitrary normed spaces.

\subparagraph{} %- properties of the weak* topology
We also introduce the \textit{weak*}-topology on the adjoint $\banX^*$
...
The main result concerning weak*-topologies in adjoint spaces is
the well-known \textit{Alaoglu theorem}:
...
Consider again the situation, where $\banX$ is a normed linear space.
It may happen
that the natural embedding
$\iota\: \banX \hookrightarrow \banX^{**}$,
$\iota(x) := \phi(x)$ for $\phi \xeof \banX^*$
is even surjective,
which in this case
constitutes a norm-isomorphism between the spaces $\banX$ and $\banX^*$.
...
\subparagraph{}
More general,
the convolution $\disF * \disG$ $\disF, \disG \xeof \D'(\Rd)$ exists,
...
%------------------------------------------------------------------------------%
%   V. NORMALLY SOLVABLE OPERATORS ---
%------------------------------------------------------------------------------%
%  §1. Banach's Closed Range Theorem ---
%  §2. Operators Bounded Below ---
%  §3. The Fredholm Index ---
%  §4. Perturbations of Semi-Fredholm Operators ---
%  §5. Basic Theory on Fredholm Operators ---
%  §6. Fredholm's Alternative Theorem ---
%  §7. Concepts of Solvability for Linear Equations ---
%  ---

\def\TitleV{Normally Solvable Operators}
\newpage
\thispagestyle{empty}
...
\section{Banach's Closed Range Theorem}
%------------------------------------------------------------------------------%
...
see \textsc{Browder} \cite{V_Br}, theorem\,1.1 and theorem\,1.2.

\section{Operators Bounded Below}
%------------------------------------------------------------------------------%
...
If $\linA$ fulfills \eqref{V_BOUNDED_BELOW_CONDITION},
...
\subparagraph{}
For a proof of this theorem, see \cite{V_Wl}, theorem 12.12, page 180.
...
%------------------------------------------------------------------------------%
{Fredholm theory} is useful in the study of linear operators
of type $\1_X+\compA$, 
where $\compA \: \banX \xto \banY$ is compact.
Remember that such operators are Fredholm with index $0$,
to which \textsc{Fredholm}'s alternative theorem can be applied.

\subparagraph{}
The operator $\1_X + \tilde{\compA}$ is Fredholm with vanishing index,
hence theorem \ref{FREDHOLM_ALTERNATIVE_THEOREM} is applicable
and consequently provides assertions
also for the solution of the equation $\linA x \eq y$.
...
\begin{itemize}[leftmargin=15pt]
\item[(a)]
\textit{densely solvable} (\cite{V_Kr}),
if $\overline{\range{\linA}} \eq \banY$;

\item[(b)]
\textit{normally solvable} (\cite{V_Kr}),
if $\overline{\range{\linA}} \eq \range{\linA}$;

\item[(c)]
\textit{correctly solvable} (\cite{V_Kr}),
if $\linA$ is bounded below;

\item[(d)]
\textit{semi-solvable} (\cite{V_Br}), if $\linA$ is Fredholm;

\item[(e)]
\textit{almost solvable} or \textit{regularly solvable} (\cite{V_Br}),
if $\linA$ is semi-solvable and $\chi(\linA) \eq 0$;

\item[(f)]
\textit{uniquely solvable} (\cite{V_Kr}),
if $\linA$ is injective;

\item[(g)]
\textit{everywhere solvable} (\cite{V_Kr}),
if $\linA$ is surjective;

\item[(h)]
\textit{solvable} (\cite{V_Br}),
if it is uniquely and everywhere solvable;
...
\item[(j)]
\textit{completely solvable} (\cite{V_Br}) or \textit{compactly solvable}
...
\bibitem{V_Br} F.\,Browder:
\textit{Functional Analysis and Partial Differential Equations I}.
Math. Annalen 138, pp.\,55-79, 1959.

\bibitem{V_Kr} S.\,Krein:
\textit{Linear Equations in Banach Spaces}.
Birk\-h\"{a}user, 1982.

\bibitem{V_Wl} J.\,Wloka:
...
%------------------------------------------------------------------------------%
%  §1. Resolvent and Spectrum of Banach Algebra Elements ---
...
%------------------------------------------------------------------------------%
Let us first have a look to the notion of resolvent and spectrum in the setting
of unital complex Banach algebras.
...
\paragraph{} %- spectrum of Banach algebra elements
...
\subparagraph{}
Hence it is worthwhile first to inspect the spectral theory
of linear transformations $\linA\:\C^d \xto \C^d$.
...
since they appear as realizations of linear-ellipic differential operators
defined in \textit{bounded} domains $\G \subset \Rd$.
...
%------------------------------------------------------------------------------%
Our subject in this section is
...
\subparagraph{}
The concept of \textsc{Riesz} projection may be extended into two directions.
First,
...
%------------------------------------------------------------------------------%
%  §1. The Gelfand Representation ---
%  §2. Involutional Algebras and C*-Algebras ---
%  §3. The Gelfand-Naimark Theorem ---
%  §4. Generalities on Topological Groups ---
%  §5. Locally Compact Abelian Groups ---
%  §6. Abstract Fourier Transformation ---
%  §7. Fourier Transformation in Euclidean Spaces ---
%  ---
...
\newpage
\thispagestyle{empty}
\fancyhead[C] {\small{\textsc{VIII. \TitleVII}}}
\part{\TitleVIII}
%------------------------------------------------------------------------------%
...
The \textsc{Hilbert-Schmidt} \textit{spectral theorem},
...
by semigroup theory and especially by \textsc{Stone}'s theorem (see section X),
...
\def\TitleX{Semigroup Theory and Evolution Equations}
\newpage
\thispagestyle{empty}
...
is is our second important theorem in this section:
...
\begin{theorem1}\label{QUASI-COERCIVE_FORMS_AND_ASSOCIATED_OPERATORS_2}
...
the theorem \ref{QUASI-COERCIVE_FORMS_AND_ASSOCIATED_OPERATORS_2}
...
by theorem \ref{QUASI-COERCIVE_FORMS_AND_ASSOCIATED_OPERATORS_2}.
...
%------------------------------------------------------------------------------%
% XII. APPLICATIONS TO PARTIAL DIFFERENTIAL OPERATORS ---
%------------------------------------------------------------------------------%
% § 1. Various Notions of Ellipticity ---
% § 2. Linear Differential Operators in Banach Spaces ---
% § 3. Closeness and Closability of Differential Operators ---
% § 4. The Fredholm Property of Properly Elliptic Operators ---
% § 5. Self-Adjoint Extensions in Hilbert Space L²(G) ---  
% § 6. Spectral Analysis and Eigenvalue Problems ---
% § 7. Cauchy Problems for Partial Differential Equations ---
% § 8. Differential Operators in Divergence Form ---
% § 9. Variational Boundary Value Problems ---
% §10. Linear Differential Operators in Distributional Spaces ---
% §11. Free Space Problems ---
% §12. Fourier Transform Methods ---

\def\TitleXII{Applications to Linear Differential Operators}
\newpage
\thispagestyle{empty}
\fancyhead[C] {\small{\textsc{XII. \TitleXII}}}
\part{\TitleXII}
%------------------------------------------------------------------------------%
This final section deals with applications of the presented theory
...
\section{Various Notions of Ellipticity}
%------------------------------------------------------------------------------%
...
\quad \for \xi = (\xi_1,\ldots,\xi_d) \xeof \Rd
\]
be the \textit{characteristic function} or the \textit{main symbol} of $\A$,
which is built up by the summands of highest order
in \eqref{XII_EVEN_ORDER_LPDO}.
...
\section{Linear Differential Operators in Banach Spaces}
%------------------------------------------------------------------------------%
...
\end{equation}
Let $\G$ again be a domain in an Euclidean space $\Rd$.
...
\]
with domain of definition $\domain{\linA}$.
The Banach space $\mathbb{E}(\G)$ hast to contain the space $\TF$ 
of test functions as a dense linear subspace.
We also require for each $u \xeof \domain{\linA}$ that
first  $\A \, u$ is well-defined by $\linA u := \A \, u$ and
second $\A \, u \xeof \mathbb{E}(\G)$ holds.
Clearly,
our first condition means that $\domain{\linA}$
contains only functions $u \: \G \xto \C$ that are sufficiently often
differentiable in some sense.
But $\domain{\linA}$ should basically include the space $\TF$ of test functions,
which implies that $\domain{\linA}$ lies dense in $\mathbb{E}(\G)$,
hence the realization $\linA$ of $\A$ appears as a densely defined operator
in $\mathbb{E}$.

\subparagraph{} %- Dirichlet boundary conditions
One often has to deal with the realization $\linA_D$
of a strongly elliptic differential operator $\A$
with order $2m$,
where the functions in the domain of definition $\domain{\linA_D}$ satisfy
the $m$ boundary conditions
\begin{equation}\label{XII_HOMOGENEOUS_DIRICHLET_SYSTEM}
\BDO_k(x,u) \eqdef (\partial_{\nu}^k u)(x) \= 0,
\quad \quad x \xeof \bndG, \quad k \eq 0,\ldots,m{-}1
\end{equation}
of a so-called \textit{homogeneous Dirichlet system}.
...
\subparagraph{} %- realizations in various Banach spaces
It is possible to use various Banach spaces,
in which the operator $\linA_D$ may act.
Let us enumerate some possibilities:

\begin{itemize}[leftmargin=12pt]
\item[1.]
$\mathbb{E}(\G) \eq \Lp$,
that is,
$\linA_D$ acts in one of the Lebesgue spaces ($1 \leq p < \infty$),
and the domain of definition is given by
\[
\domain{\linA_D} \eqdef \{ u \in \WN{m,p}{\G}: \A\,u \in \Lp \}.
\]
\item[2.]
$\mathbb{E}(\G) \eq C^{\gamma}(\clG)$,
so $\linA_D$ acts in a {\HOELDER} space ($0 \leq \gamma < 1$),
and the domain of definition is given by
\[
\domain{\linA_D} \eqdef \{ u \in C^{2m,\gamma}{\G}: 
(\partial_{\nu}^k u)(x) = 0 \for k \eq 0,\ldots,m{-}1 \}.
\]
\item[3.]
$\mathbb{E}(\G) \eq C(\clG)$,
and the domain of definition $\domain{\linA_D}$ is given in this case
by the functions $u \in C(\clG)$,
for which 
\begin{itemize}[leftmargin=30pt,itemsep=5pt]
\item[(a)]
$u \in \bigcap_{\,p \ge 1} W_{loc}^{2m,p}(\G)$,
\item[(b)]
$\A\,u \in C(\clG)$,
\item[(c)]
$(\partial_{\nu}^k u)(x) \eq 0 \for k \eq 0,\ldots,m{-}1$.
\end{itemize}
\end{itemize}

\section{Closeness and Closability of Differential Operators}
%------------------------------------------------------------------------------%
We next turn to the question of closeness of the Dirichlet operator $\linA_D$
acting in the Lebesgue space $\Lp$.
Let the domain $\G$ and the differential expression $\A$
...
where $\abs{u} _p$ is the Sobolev norm of $u$ in $\Lp$,
theorem \ref{II_BROWDER_THEOREM} implies that the operator $\linA_D$ is closed.
Moreover,
theorem \ref{V_PEETRE_THEOREM} shows that
$\linA_D$ is injective and the range $\range{\linA_D}$
is a closed subspace in $\Lp$.

\paragraph{} %- a criterion for closability of linear differential operators
As already mentioned,
...
\begin{theorem1}\label{XII_CLOSABILITY_THEOREM}
...
of the operator $\linA$ considered in theorem \ref{XII_CLOSABILITY_THEOREM}
is not restricted to any boundary conditions.
It should also be mentioned that
rather the closability of the differential operator $\linA$
with domain of definition $\domain{\linA}$
like in theorem \ref{XII_CLOSABILITY_THEOREM} is ensured,
...
\section{The Fredholm Property of Properly Elliptic Operators}
%------------------------------------------------------------------------------%
This feature is valid for properly linear-elliptic differential operators
...
whose underlying domain is \textit{bounded}.
...
\section{Self-Adjoint Extensions in Hilbert Space L²(G)}
%------------------------------------------------------------------------------%
We denote a linear differential expression $\DO$ \textit{formally self-adjoint}
in case of $\DO^{\dag} \eq \DO$.
If $\DO$ has constant coefficients,
then it is formally self-adjoint
iff
$c_{\alpha} \in \R$ for $\abs{\alpha} $ even
and $c_{\alpha} \in i \, \R$ for $\abs{\alpha} $ odd.

\paragraph{} %- examples of self-adjoint realizations
In the rest of this section we want to enumerate a couple
of formally self-adjoint differential expression
and elaborate the self-adjoint extensions related to them.
More precisely,
we mention the momentum operator, the Sturm-Liouville operator, 
the Laplacian and the {\SCHROEDINGER} operator.

\begin{itemize}[leftmargin=12pt,itemsep=5pt]
\item[1.] \textit{The Momentum Operator on Various Intervals.}\\
For an arbitrary interval $\J \subset \R$
...
\subparagraph{}
Basically,
for each $\lambda \in \C$ the function $f_{\lambda}(x) := e^{-i \lambda x}$
...
\paragraph{} % self-adjointness in the real line
...
For $f,g \in \domain{\linA_1}$ we compute
\[
\scalar{\linA_1 f}{g}
\=
...
We have $u \in D_+ := \kernel{\overline{\linA_1}\,^*-i}$, iff
...
\subparagraph{}
Similarly,
$v \in D_- := \kernel{\overline{\linA_1}^{\,*} +i}$ iff
...
\item[2.] \textit{The Sturm-Liouville Operator.}\\
...
\quad \quad x \in (a,b)
\]
with $p(x) > 0$ and $q(x) \geq 0$ for $x \in [a,b]$,
...
$u \in C[a,b]$ with $u(a) \eq u(b) \eq 0$.
...
Especially for $p(x)=1$ and $q(x) \eq 0$ for $x \in (a,b)$ it is possible
...
\item[3.] \textit{The Laplacian in Various Domains $\G \subset \Rd$.}\\
...
and due to the boundary condition $\phi(x) \eq 0$ for $x \in \partial \G$
and $\phi \in C_c^{\infty}(\G)$,
we compute for $\phi,\psi \in C_c^{\infty}(\G)$
\[
...
It should be mentioned 
that when working with test functions $\phi \in C_0^{\infty}(\G)$
there was no need
...
\[
m_2:(x_1,\ldots,x_n) \mapsto (x_1^2 + \ldots + x_n^2), \quad x \in \Rd
\]
...
$\phi \in \domain{-\Delta_F}$ is given by the formula
\[
...
which in case of $\G \eq \Rd$ is identical to the space $H^1(\Rd)$.

\subparagraph{}
The domain of definition of the adjoint operator $-\Delta^*$ is the subspace
\[
\domain{-\Delta^*} \= \{f \in L^2(\Rd): -\Delta f \in L^2(\Rd)\},
\]
...
\item[b.]
\textit{The Dirichlet-Laplacian}.
Let $\G \subset \Rd$ be a \textit{bounded} domain.
...
associated with $\overline{Q}$ and defined by
\[
\begin{cases}[1.3]
\quad \domain{\linA_D} & :=
\{f \in \HN{1}{\G}: \Delta f \in L^2(\G)\} \\
\quad \linA_D f & := \,
- \Delta f, \quad f \in \domain{\linA_D}. \\
\end{cases}
\] 
The operator $\linA_D$ is called the
\textit{Laplace operator with Dirichlet boundary conditions},
often it is denoted the \textit{Dirichlet-Laplacian}.
Obviously,
we have
\[
...
Let $\G \subset \Rd$ be a domain with $C^1$-boundary $\partial \G$,
let $\hilH$ be the Hilbert space $\Lh$ and let the operator $\linA_N$
be defined by
\[
\begin{cases}[1.3]
\quad \domain{\linA_N} & := \,
\{ f \in H^2(\G): f_v(x)= 0, x \in \partial \G \} \\
\quad \linA_N f & := \,
- \Delta f, \quad f \in \domain{\linA_N}. \\
\end{cases}
\]
...
\item[4.] \textit{The {\SCHROEDINGER} Operator.}\\
...
As an application of the Kato-Rellich theorem coming from perturbation theory
(see section VIII, \S\,7),
consider the case,
...
of problem \eqref{XII_SCHROEDINGER_EIGENVALUE_PROBLEM}.
...
\section{Spectral Analysis and Eigenvalue Problems}
%------------------------------------------------------------------------------%
...
\section{Cauchy Problems for Partial Differential Equations}
%------------------------------------------------------------------------------%
Let us consider the Laplacian in the entire space $\Rd$.
Then theorem \ref{SEMIGROUP_CREATED_BY_SECTORIAL_OPERATOR_IS_ANALYTIC} ensures
...
\section{Differential Operators in Divergence Form}
%------------------------------------------------------------------------------%
We now want to illustrate the usage of abstract sesquilinear
...
can be transformed into an expression $\A$
in divergence form \eqref{XII_LPDO_DIV},
...
\begin{theorem1}\label{XII_GARDING_INEQUALITY_THEOREM_2}
...
\section{Variational Boundary Value Problems}
%------------------------------------------------------------------------------%
...
\end{cases}
\]
Following \textsc{Lions}'s theorem on parabolic equations,
...
then \textsc{Lion}'s theorem on hyperbolic initial-value problems implies
