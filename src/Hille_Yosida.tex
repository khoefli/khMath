...
when a linear operator $\linA$ acting in a complex Banach space $\banX$
...
%------------------------------------------------------------------------------%
Let $\banX$ be a complex Banach space and $\BO{\banX}$ be the linear space
of bounded linear operators $\linS \: \banX \xto \banX$
with identity operator $\1_{\banX}$.
By a \textit{C$_0$-semigroup} or
...
But it is even differentiable under a very mild assumption:
For each $x \xeof \banX$
the mapping $\eta_x$ is differentiable in $(0,\infty)$
iff
it is right differentiable in $t \eq 0$ only.
...
that is,
\[
\domain{\linA} \eqdef \{x \xeof \banX \: \eta_x \mbox{ is differentiable}\}.
\]
It is not hard to show that
$\domain{\linA}$ is a linear subspace of $\banX$
and the mapping $\linA$ is linear,
so $\linA$ constitutes a linear operator acting in $\banX$.

\paragraph{} %- theorem 3.1
The following theorem contains a couple of assertions,
which will be used in proving the main theorems of this text.

\begin{theorem}\label{XXX}
Let {\SG} be a C$_0$-semigroup and let $\linA$ be its generator.
Then
...
First, a linear operator $\linA$ is said to be densely defined in $\banX$,
if $\domain{\linA}$ lies dense in $\banX$.
Second, the operator $\linA$ is called \textit{closed},
if the set
\[
\Gamma_{\linA} \eqdef
...
that a linear operator $\linA$ is closed
...
then every operator $\zeta-\linA$ ($\zeta \xeof \C$)
...
$\linA^{-1}\: \range{\linA} \xsubset \banY \xto \domain{\linA} \xsubset \banX$
...
Our next theorem indicates that infinitesimal generators fit into the
class of densely defined and closed linear operators:

\begin{theorem}\label{YYY}
...
which implies $\overline{\domain{\linA}} \eq \banX$.

\subparagraph{}
To show that $\linA$ is closed,
let $x_n \xeof \domain{\linA}$ with $x_n \xto x$ and $\linA x_n \xto y$.
By \ref{XXX}\,(iv) we have that
\begin{equation}\label{YYY_1}
S(t)x_n -x_n \= \int_0^t S(t)\,\linA x_n \, ds.
\end{equation}
The integrand converges to $S(s)y$ uniformly on bounded intervals.
Thus letting $n \to \infty$ in \eqref{YYY_1}, we obtain
\[
...
Using the fact that an infinitesimal generator is densely defined,
we are able to quote the following uniqueness theorem:

\begin{theorem}
If $\linA$ and $\linB$ are generators of C$_0$-semigroups,
then $\linA \eq \linB$ imply that the generated semigroups are equal, too.
\end{theorem}
...
of the equation $\dot{x} \eq \linA x$
satisfying the initial value condition $x(0) \eq x_0$.

\paragraph{} %- solution concepts for the Cauchy problem
First of all we have to define concepts of solution for this problem:
...
that problem \eqref{HOMOGENEOUS_CAUCHY_PROBLEM}
iff
it is correctly posed and $\linA$ is regular,
that is,
$\rho(\linA) \ne \emptyset$ holds.

\paragraph{}
The main theorem of this section claims the following statement
...
%------------------------------------------------------------------------------%
In this section we want to introduce some concepts of spectral theory.
...
strongly continuous semigroups and the resolvents of their generating operators,
called the \textit{Laplace transform}.
...
\paragraph{}
Let us put together the necessary conditions for $\linA$ to be an infinitesimal
generator:
...
\for \zeta \in (\omega,\infty) \and n \xeof \N.
\end{equation}
\end{itemize}
\end{corollary}
...
%------------------------------------------------------------------------------%
As it has already been elaborated in the last section,
to each strongly continuous semigroup {\SG} there is a linear operator $\linA$
as its infinitesimal generator.
Reversely,
the question arises,
under which conditions a linear operator forms
...
we shall reformulate the theorem for a special case and come back to the proof
at the end of this section.
\qed

\paragraph{} %- quasi-contractive semigroups
In fact,
before proving theorem \ref{GENERAL_HY-THEOREM},
let us especially consider the case $M \eq 1$.
If the C$_0$-semigroup {\SG} is of type $(1,\omega)$,
then the estimation $\norm{S(t)} \leq e^{\omega t}$ holds
for all $t \geq 0$,
and the semigroup is denoted a \textit{semigroup of quasi-contractions}.
Then the family \eqref{HILLE-YOSIDA_CONDITIONS} of resolvent estimates
can be reduced to one single estimate,
namely for that with $n \eq 1$,
and theorem \ref{GENERAL_HY-THEOREM} simplifies
to the classical \textit{Hille-Yosida} \textit{theorem}:
